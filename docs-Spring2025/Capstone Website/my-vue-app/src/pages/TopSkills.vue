...
export default {
  ...
  methods: {
    ...
    async fetchJobFields() {
      try {
        const response = await fetch("https://itskills.onrender.com/get_job_fields");
  ...
  try {
    ...
    const chartResponse = await fetch("https://itskills.onrender.com/top_skills_per_field", {
      method: "POST",
      headers: {
    ...
    // Fetch courses data
    const coursesResponse = await fetch("https://itskills.onrender.com/courses_for_field", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
